...
function isSequence(cards) {
  ...
  return isConsecutive(unique) || isLowAceSequence(cards);
}

function isConsecutive(ranks) {
  const unique = [...ranks].sort((a, b) => a - b);
...
function isLowAceSequence(cards) {
  if (!cards.some(c => c.rank === 'A')) return false;

  const lowAceRanks = cards.map(c => (c.rank === 'A' ? -1 : RANK_ORDER[c.rank]));
  return isConsecutive(lowAceRanks);
}
...
module.exports = { calculateSum, isSequence, isValidDiscard, removeCards };
