...
function isRegisteredProfileComplete(user) {
  return ['first_name', 'nickname', 'country_id', 'dob'].every((field) => hasProfileValue(user?.[field]));
}

function getMissingProfileFields(user) {
  return ['first_name', 'nickname', 'country_id', 'dob'].filter((field) => !hasProfileValue(user?.[field]));
}
