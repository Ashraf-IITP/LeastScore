...
function isRegisteredProfileComplete(user) {
  return ['first_name', 'nickname', 'country_id', 'dob', 'gender'].every((field) => hasProfileValue(user?.[field]));
}

function getMissingProfileFields(user) {
  return ['first_name', 'nickname', 'country_id', 'dob', 'gender'].filter((field) => !hasProfileValue(user?.[field]));
}
